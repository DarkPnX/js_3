...
/*
 ...
 3.3: Необходимо выбрасывать исключение в случаях:
   - fn не является функцией (с текстом "fn is not a function")
 */
function returnBadArguments(fn,...elems) {
    if(typeof fn !== 'function'){
        throw 'fn is not a function';
    }
    let buffArray=[];
...
function calculator(number=0) {
    ...
    let obj={
        sum:(...elems)=>elems.reduce((summ,val)=>{return summ + val},number),
        dif:(...elems)=>elems.reduce((summ,val)=>{return summ - val},number),
        div:(...elems)=>elems.reduce((summ,val)=>{
            if(val===0){
                throw "division by 0";
            }
            return summ / val
        },number),
        mul:(...elems)=>elems.reduce((summ,val)=>{return summ * val},number)
    }
    return obj;
}
